...
```{r, echo=TRUE}
# 1. Preparar os dados para o gráfico de degrau
binned_data_steps <- oster %>%
  filter(bmi < 100) %>%
  mutate(bmi_cut = cut(bmi, 8)) %>%
  group_by(bmi_cut) %>%
  summarize(mean_heart_health = mean(heart_health, na.rm = TRUE)) %>%
  ungroup() %>%
  # Extrair os limites numéricos de início e fim de cada faixa
  mutate(
    interval_str = as.character(bmi_cut),
    clean_str = str_replace_all(interval_str, "\\[|\\]|\\(|\\)", ""),
    endpoints = str_split(clean_str, ",", simplify = TRUE),
    x_start = as.numeric(endpoints[, 1]),
    x_end = as.numeric(endpoints[, 2])
  )

# 2. Gerar o gráfico com segmentos horizontais
p <- ggplot(binned_data_steps, 
       aes(y = mean_heart_health)) +
  # Adiciona um scatterplot de fundo para dar contexto
  geom_point(data = oster %>% filter(bmi < 100) %>% sample_n(500), 
             aes(x = bmi, y = heart_health), 
             alpha = 0.15, color = "gray") +
  # Desenha os segmentos horizontais para cada bin
  geom_segment(aes(x = x_start, xend = x_end, yend = mean_heart_health),
               color = "firebrick", size = 1.5) +
  labs(
    title = "Função de Degrau Gerada pelo Binning",
    subtitle = "A média condicional é constante em cada faixa de IMC",
    x = "Índice de Massa Corporal (IMC)",
    y = "Média de Saúde Cardíaca"
  ) +
  theme_pubr()
```
---
...
---
## O Teorema de Frisch-Waugh-Lovell (Enunciado e Prova)

O Teorema de Frisch-Waugh-Lovell (FWL) formaliza a intuição de "manter Z constante". Ele nos dá um método alternativo e conceitualmente poderoso para calcular os coeficientes de uma regressão múltipla.

**Enunciado do Teorema:**
Considere o modelo de regressão múltipla particionado em dois conjuntos de regressores, $X_1$ e $X_2$:
$$ Y = X_1\beta_1 + X_2\beta_2 + \varepsilon $$
O estimador de Mínimos Quadrados Ordinários (MQO) para $\beta_1$, denotado como $\hat{\beta}_1$, pode ser obtido de duas formas:
1.  **Regressão Múltipla Completa:** Regredindo $Y$ em $X_1$ e $X_2$ simultaneamente.
2.  **Regressão em Etapas (FWL):**
    a. Regrida $Y$ em $X_2$ e obtenha os resíduos, $\tilde{Y} = Y - X_2\hat{\gamma}_Y$. ($\tilde{Y}$ é a parte de $Y$ *não explicada* por $X_2$).
    b. Regrida cada coluna de $X_1$ em $X_2$ e obtenha os resíduos, $\tilde{X}_1 = X_1 - X_2\hat{\Gamma}_X$. ($\tilde{X}_1$ é a parte de $X_1$ *não explicada* por $X_2$).
    c. Regrida os resíduos $\tilde{Y}$ nos resíduos $\tilde{X}_1$. O coeficiente desta regressão simples é numericamente idêntico a $\hat{\beta}_1$ da regressão múltipla completa.
---

**Prova (Esboço):**
Seja $M_2 = I - X_2(X_2'X_2)^{-1}X_2'$ a matriz que projeta um vetor no espaço ortogonal a $X_2$ (a "matriz residualizadora"). Os resíduos das etapas (a) e (b) são $\tilde{Y} = M_2Y$ e $\tilde{X}_1 = M_2X_1$.
A regressão de $\tilde{Y}$ em $\tilde{X}_1$ na etapa (c) nos dá o estimador:
$$\hat{\beta}_{1,FWL} = (\tilde{X}_1'\tilde{X}_1)^{-1}\tilde{X}_1'\tilde{Y} = ((M_2X_1)'(M_2X_1))^{-1}(M_2X_1)'(M_2Y)$$
Como $M_2$ é simétrica ($M_2' = M_2$) e idempotente ($M_2M_2 = M_2$), a expressão se simplifica para:
$$\hat{\beta}_{1,FWL} = (X_1'M_2X_1)^{-1}X_1'M_2Y$$
Esta é exatamente a fórmula para o subvetor $\hat{\beta}_1$ obtido pela inversão da matriz de covariância particionada do modelo de regressão completo. Q.E.D.

---
## Conexão do FWL com Planejamento Experimental

O Teorema FWL não é apenas uma curiosidade matemática; ele é a base para entendermos por que e como usamos variáveis de controle (covariadas) em análises de experimentos.

**1. O Poder da Aleatorização e a Ortogonalidade:**
Em um **experimento completamente aleatorizado**, a atribuição do tratamento ($X_1$) é, por construção, estatisticamente independente de qualquer característica pré-tratamento das unidades experimentais (as covariadas, $X_2$). Isso implica que $X_1$ e $X_2$ são (em média, na população) **ortogonais**.
*   **O que isso significa na prática?** Regredir $X_1$ em $X_2$ (etapa b do FWL) resultará em coeficientes próximos de zero. Os resíduos $\tilde{X}_1$ serão quase idênticos a $X_1$.
*   **Implicação:** Em um experimento aleatorizado, o coeficiente do tratamento $\hat{\beta}_1$ da regressão simples ($Y$ em $X_1$) será muito similar ao da regressão múltipla ($Y$ em $X_1$ e $X_2$). A inclusão de covariadas não causa viés, nem é estritamente necessária para obter uma estimativa não-viesada do efeito do tratamento.
---

**2. Por que incluir covariadas, então? Para Aumentar a Precisão!**
Se a covariada $X_2$ (ex: uma medida base do resultado) explica uma parte da variação em $Y$, então o resíduo $\tilde{Y}$ (etapa a do FWL) terá uma variância menor que a variância do $Y$ original.
*   **O que isso significa?** Ao regredir $\tilde{Y}$ em $\tilde{X}_1$, estamos explicando a variação do "resultado limpo" (sem a influência de $X_2$) usando a variação do "tratamento limpo".
*   **Implicação:** Isso reduz a variância do erro do modelo final ($\sigma^2_{\varepsilon}$), o que leva a erros padrão menores para $\hat{\beta}_1$. Ganhamos **poder estatístico** para detectar efeitos do tratamento que, de outra forma, poderiam ser mascarados pelo "ruído" nos dados.

**Em resumo:** O Teorema FWL nos mostra que adicionar covariadas a um modelo de experimento aleatório "limpa" a variável de resultado, permitindo um teste mais preciso e poderoso do efeito do tratamento, sem enviesar a estimativa do efeito em si. Essa técnica é conhecida como **ANCOVA (Análise de Covariância)**.
...
```{r, echo = TRUE, eval = TRUE}
modelsummary(list("Com Controle (Z)" = m2, "FWL (Resíduos)" = m3), stars = TRUE, fmt = 5, 
             gof_omit = 'AIC|BIC|Lik', output = "data.frame") %>%  knitr::kable()
```
...
*   $\mu$ é a média geral de todas as observações (o "grande intercepto").
*   $\alpha_i$ é o **efeito do tratamento $i$**. Pense nisso como o "empurrão" (positivo ou negativo) que o tratamento *i* dá à média geral. É a diferença entre a média do grupo $i$ e a média geral ($\alpha_i = \mu_i - \mu$).
*   $\epsilon_{ij}$ é o erro aleatório, que se assume ter distribuição Normal com média 0 e variância $\sigma^2$.
---

**Onde estão as "covariadas" neste modelo?**
Esta é uma pergunta natural e muito importante. A resposta é que **as próprias designações de grupo são as covariadas!** O modelo ANOVA é uma forma de escrever um modelo de regressão onde os preditores são variáveis categóricas (os tratamentos).

Podemos reescrever o modelo acima usando variáveis *dummy* (indicadoras):
$$Y_{ij} = \beta_0 + \beta_1 D_{1} + \beta_2 D_{2} + \dots + \beta_{k-1} D_{k-1} + \epsilon_{ij}$$
Onde $D_1, D_2, \dots$ são variáveis que valem 1 se a observação pertence àquele grupo, e 0 caso contrário.

*   $\beta_0$ corresponde à média do grupo de referência.
*   Cada $\beta_i$ corresponde à *diferença* entre a média do grupo *i* e a média do grupo de referência.
*   Os $\alpha_i$ do modelo ANOVA são apenas uma reparametrização dos $\beta_i$ da regressão.

Portanto, a ANOVA não ignora covariadas; ela é projetada especificamente para o caso em que a principal "covariada" de interesse é o próprio tratamento (uma variável categórica).
...
---
## ANOVA, Bloqueio e o Teorema FWL

A conexão entre ANOVA e regressão se torna ainda mais poderosa quando consideramos desenhos experimentais mais complexos, como o **Desenho em Blocos Aleatorizados**.

Nesse desenho, agrupamos unidades experimentais em "blocos" homogêneos e, em seguida, aleatorizamos os tratamentos *dentro* de cada bloco. O objetivo é controlar uma fonte conhecida de variação (o efeito do bloco).

O modelo de uma ANOVA de dois fatores (tratamento e bloco) é:
$$Y_{ij} = \mu + \alpha_i (\text{efeito do tratamento}) + \rho_j (\text{efeito do bloco}) + \epsilon_{ij}$$
---

**Como a ANOVA encontra o efeito "puro" do tratamento, $\alpha_i$?**
Aqui, o Teorema de Frisch-Waugh-Lovell nos dá a intuição perfeita:
1.  A ANOVA primeiro "remove" a variação em $Y$ que é explicada pelo fator de bloqueio $\rho_j$. Isso é análogo a obter os resíduos $\tilde{Y}$ da regressão de $Y$ nas variáveis *dummy* do bloco.
2.  Como o tratamento foi aleatorizado *dentro* dos blocos, o tratamento $\alpha_i$ e o bloco $\rho_j$ são ortogonais. Isso significa que a parte do tratamento não explicada pelos blocos ($\tilde{X}_1$) é o próprio tratamento.
3.  A ANOVA então calcula o efeito do tratamento sobre esses resultados "ajustados para o bloco".

Em suma, a ANOVA em um desenho em blocos é uma aplicação direta do princípio FWL: ela estima o efeito do tratamento **depois de controlar pela variabilidade entre os blocos**, fornecendo uma estimativa mais precisa e poderosa do efeito causal de interesse.
