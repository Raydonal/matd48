...
- **O quê:** O foco da nossa aula. Aqui, o investigador **manipula ativamente** uma variável (o fator) para observar o seu efeito noutra variável (a resposta), enquanto tenta controlar todas as outras.
...
- **Limitações:** Nem sempre são viáveis ou éticos (não podemos forçar pessoas a fumar).
...
- **A Afirmação:** Uma senhora afirmava conseguir distinguir, pelo sabor, se o leite ou o chá foram colocados primeiro na chávena.
...
- **O Delineamento Genial de Fisher:**
    ...
    2. Apresentar as 8 chávenas à senhora em **ordem aleatória**.
    3. Pedir que ela separe as chávenas nos dois grupos de 4.
...
```{r nixon-sim, message=FALSE, warning=FALSE}
set.seed(123)
...
```{r nixon-tukey-plot, echo=FALSE, fig.align='center', warning=FALSE}
