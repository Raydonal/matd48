...
---
### Solução: O Delineamento em Blocos Completos Aleatorizados (RCBD)

A solução é o **bloqueio**.
1.  Dividimos o campo em 4 **Blocos**, perpendiculares ao gradiente. Cada bloco é uma área considerada homogênea *internamente*.
2.  Em **cada bloco**, aplicamos *todos* os 3 tratamentos a 3 unidades experimentais, de forma aleatória.

**Análise das Relações:**
- Fator de interesse: `Tratamento`.
- Fator de bloqueio (controle de ruído): `Bloco`.
- Como cada tratamento aparece em cada bloco, os fatores são **cruzados**.
- A interação `Bloco:Tratamento` está confundida com o erro experimental ($\epsilon$).

---
### Simulação e Visualização: O Problema do "Ruído"

Vamos simular os dados. O Tratamento B é o melhor (+5), mas o Bloco 4 é muito mais fértil (+12).
```{r}
set.seed(99)
dados_rcbd <- data.frame(Bloco = factor(rep(1:4, each = 3)),
  Tratamento = factor(rep(c("A", "B", "C"), times = 4))
)
# Definindo efeitos REAIS
efeito_bloco <- c(0, 4, 8, 12)  # Blocos cada vez mais férteis
efeito_trat <- c(0, 5, 0)      # Tratamento B é o melhor
# A resposta é a soma dos efeitos + ruído
dados_rcbd <- dados_rcbd %>%
  mutate(
    Resp_Base = 20 + efeito_bloco[as.numeric(Bloco)] + efeito_trat[as.numeric(Tratamento)],
    Resposta = round(Resp_Base + rnorm(12, 0, 1.5), 1)
  )
```
---
**Visualização 1: Ignorando os Blocos (Análise Incorreta de um DIC)**
Se analisarmos os dados como se fossem de um Delineamento Inteiramente Casualizado (DIC), a variabilidade dos blocos obscurece tudo.
```{r, fig.height=4.5, echo=FALSE}
...
```
**Interpretação:** A sobreposição entre os boxplots é enorme. Uma ANOVA incorreta (`lm(Resposta ~ Tratamento)`) provavelmente não detectaria a superioridade do Tratamento B.

---
### O Diagrama de Hasse nos Mostra o Caminho

O diagrama de Hasse para um RCBD nos diz que `Bloco` e `Tratamento` são fatores cruzados.

.pull-left[
**Diagrama de Hasse do RCBD:**
```{r, echo=FALSE, fig.height=3.5}
grViz("
digraph rcbd {
  rankdir=TB;
  node [shape=box, style=rounded]; edge [arrowhead=none];
  mu [label='μ']; bloco [label='Bloco']; trat [label='Tratamento'];
  epsilon [label='ε \\n(Bloco:Tratamento)', shape=ellipse];
  mu -> {bloco, trat}; {bloco, trat} -> epsilon;
}
")
```
]
...
O diagrama nos força a reconhecer a existência do fator `Bloco`. Para controlar seu efeito, devemos incluí-lo no modelo. Como não estamos interessados na interação, usamos um modelo aditivo.

**Fórmula Correta do Modelo em R:**
...
dados_rcbd$Resposta_Ajustada <- predict(modelo_rcbd, type="terms")[, "Tratamento"] + 
                                mean(dados_rcbd$Resposta)
ggplot(dados_rcbd, aes(x = Tratamento, y = Resposta_Ajustada, fill = Tratamento)) +
  geom_boxplot(alpha=0.6) + geom_jitter(width=0.1) +
  labs(title="Análise Correta: Resposta Ajustada pelo Bloco",
       subtitle="O efeito real do Tratamento B agora é claramente visível") +
  theme_minimal(base_size=14) + theme(legend.position="none") +
  ylab("Resposta Ajustada")
```
**Interpretação:** Magia! Ao controlar estatisticamente a variabilidade dos blocos, a variabilidade *dentro* de cada tratamento diminui drasticamente, e a superioridade do Tratamento B torna-se inegável. O Diagrama de Hasse nos levou ao modelo que permitiu esta "limpeza" dos dados.

---
### O Diagrama de Hasse como Ferramenta de Diagnóstico *a Priori*

O poder real do diagrama é no **planejamento** de um experimento.

**Cenário de Diagnóstico:** Um pesquisador propõe um estudo para comparar `Método de Ensino` X e Y. Ele decide que o `Professor` Silva usará o Método X e a Profa. Santos usará o Método Y.

**Passo 1: Desenhar o Diagrama de Hasse do delineamento proposto.**
- `Professor` e `Método` não são cruzados (Prof. Silva nunca usa Y).
- `Professor` e `Método` não são aninhados (Método X não está "dentro" do Prof. Silva de uma forma hierárquica).
- Eles são **confundidos**: saber o professor implica saber o método.
---

**Passo 2: Diagnóstico a partir do Diagrama.**
...
digraph confounded {
  ...
  mu [label = 'μ']; epsilon [label = 'ε']; conf [label='(Professor, Método)'];
  mu -> conf -> epsilon;
}
")
```

**Diagnóstico:** O diagrama revela imediatamente a falha. Os dois fatores ocupam o mesmo nó, significando que seus efeitos são inseparáveis. O experimento, como planejado, **não pode responder à pergunta de pesquisa**.

**Passo 3: Ação Corretiva.**
O diagrama nos força a redesenhar o experimento. A solução óbvia é **cruzar** os fatores: ambos os professores devem usar ambos os métodos. O novo diagrama seria o de fatores cruzados, que é um delineamento válido.
