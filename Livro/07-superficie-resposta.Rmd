...
```{r setup-cap7-rsm, include=FALSE}
...
$R^2$ ajustado do modelo (`r round(summary(modelo_rsm)$adj.r.squared, 3)`) confirma que a
...
```{r energia-superficie, fig.width=7, fig.height=6, echo=FALSE}
grade <- expand_grid(
...
grade$energia_pred <- predict(modelo_rsm, newdata = grade)
matriz_z <- matrix(grade$energia_pred, nrow = 40, ncol = 40)

persp(
  x = seq(min(energia$Velocidad), max(energia$Velocidad), length.out = 40),
  y = seq(min(energia$angulo), max(energia$angulo), length.out = 40),
  z = matriz_z, theta = -35, phi = 20, expand = 0.6, col = "#cfe8ef",
  xlab = "Velocidade", ylab = "Ângulo", zlab = "Energia (predita)",
  main = "Superfície de resposta ajustada: energia de corte", ticktype = "detailed"
)
```

```{r energia-contorno, fig.width=6.5, fig.height=5, echo=FALSE}
ggplot(grade, aes(x = Velocidad, y = angulo, z = energia_pred)) +
  geom_contour_filled(alpha = 0.85) +
  geom_point(data = energia, aes(x = Velocidad, y = angulo), inherit.aes = FALSE,
             color = "white", size = 1.5) +
  labs(x = "Velocidade de corte", y = "Ângulo de saída", fill = "Energia\npredita",
       title = "Curvas de nível da superfície ajustada") +
...
$\lambda_1\approx$ `r signif(lambda_canonico[1], 3)` e $\lambda_2\approx$
`r signif(lambda_canonico[2], 3)` é a mesma informação da Seção \@ref(ponto-estacionario) (sela),
...
$|\lambda_2|/|\lambda_1| \approx$ `r round(abs(lambda_canonico[2]) / abs(lambda_canonico[1]))`,
...
em que $\mathbf{x}_0=(`r round(ponto_estacionario[1], 2)`\text{ m/s};\ `r round(ponto_estacionario[2], 1)`^\circ)$
...
```{r energia-ridge, fig.width=6.5, fig.height=5}
...
```{r steepest-ascent-plot, echo=FALSE, fig.width=9, fig.height=4}
p_caminho_espaco <- ggplot(caminho_subida, aes(temperatura, tempo, color = retencao_verdadeira)) +
  geom_path(linewidth = 1, color = "grey40") +
  geom_point(size = 3) +
  scale_color_viridis_c(name = "Retenção\nde vit. C (%)") +
  labs(title = "Caminho no espaço dos fatores", x = "Temperatura (°C)", y = "Tempo (h)") +
  theme_minimal(base_size = 12)

p_caminho_resposta <- ggplot(caminho_subida, aes(passo, retencao_verdadeira)) +
  geom_line(color = "grey40") +
  geom_point(aes(color = retencao_verdadeira), size = 3) +
  scale_color_viridis_c(guide = "none") +
  labs(title = "Retenção observada a cada passo", x = "Passo ao longo do caminho",
       y = "Retenção de vit. C (%)") +
  theme_minimal(base_size = 12)

cowplot::plot_grid(p_caminho_espaco, p_caminho_resposta, ncol = 2, rel_widths = c(1.15, 1))
```
...
$\alpha = (2^k)^{1/4}$ (para $k=2$, $\alpha=$ `r round((2^2)^(1/4), 3)`); um desenho rotacionável
...
```{r ccd-simulacao, fig.width=5.5, fig.height=5}
set.seed(2026)
...
```{r ccd-desejabilidade, fig.width=6.5, fig.height=5}
...
esse ponto corresponde a temperatura $\approx$ `r round(65 + 10 * melhor_ponto$x1[1], 1)`°C e
tempo $\approx$ `r round(6 + 1.5 * melhor_ponto$x2[1], 1)` h, com desejabilidade global
$D\approx$ `r round(melhor_ponto$D[1], 2)`.
...
```{r robusto-plot, fig.width=6.5, fig.height=5}
...
temperatura $\approx$ `r round(180 + 10*ponto_robusto$x1[1], 1)`°C (baixa) e pressão $\approx$
`r round(40 + 5*ponto_robusto$x2[1], 1)` (próxima do centro), com variância prevista
$\hat\sigma^2\approx$ `r round(ponto_robusto$var_pred[1], 2)` — bem abaixo da variância observada em
