...
```{r hasse-dca, out.width="42%", fig.cap="Diagrama de Hasse de um DCA com t=4 tratamentos e n=6 réplicas por tratamento (N=24). Uma cadeia simples: cada termo refina exatamente o termo anterior, sem ramificação — a assinatura de um delineamento de um único fator, sem estrutura de bloqueio."}
...
```{r hasse-axb, out.width="62%", fig.cap="Diagrama de Hasse de um fatorial A×B cruzado, a=3 níveis de A, b=4 níveis de B, n=2 réplicas por casela (N=24). A e B aparecem no mesmo nível (cruzados, sem relação de refinamento entre si) e se reencontram na interação A×B, que os refina a ambos."}
nos_axb <- tibble(
...
```{r plot-api-geral, echo=FALSE, fig.width=7.5, fig.height=4.2, fig.cap="Tempo de resposta em função da carga, por servidor, com retas de mínimos quadrados ajustadas separadamente em cada grupo."}
ggplot(dados_api, aes(x = requisicoes, y = tempo_resposta, color = servidor)) +
  geom_point(alpha = 0.65, size = 1.8) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 0.9) +
  scale_color_manual(values = c(A = "#2e8b57", B = "#0B4F6C", C = "#d98324")) +
...
```{r X-explicito, echo=FALSE}
X_ilustra <- model.matrix(~ requisicoes, data = dados_api)[1:4, ]
rownames(X_ilustra) <- paste0("obs. ", 1:4)
X_ilustra %>%
  kable(digits = 0, caption = "As quatro primeiras linhas da matriz de delineamento X (modelo simples)") %>%
  kable_styling(full_width = FALSE)
```
...
`r round(coef(mod_simples)[2], 2)` ms no tempo de resposta.
...
```{r plot-residuos-fitted, echo=FALSE, fig.width=6.5, fig.height=4, fig.cap="Resíduos contra valores ajustados do modelo simples (tempo de resposta ~ requisições)."}
tibble(ajustado = fitted(mod_simples), residuo = e) %>%
  ggplot(aes(x = ajustado, y = residuo)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  geom_point(alpha = 0.65, color = "#3b5bdb") +
  geom_smooth(se = FALSE, method = "loess", formula = y ~ x, color = "#d98324", linewidth = 0.8) +
...
```{r fig-geometria-projecao, echo=FALSE, fig.width=6, fig.height=5, fig.cap="Geometria da projeção ortogonal: o espaço-coluna de X é um subespaço de R^n (aqui representado, em corte esquemático, por uma reta); Y é um vetor fora desse subespaço; Y-chapéu é o ponto do subespaço mais próximo de Y; e o resíduo e = Y - Y-chapéu é ortogonal ao subespaço inteiro (marcado pelo ângulo reto)."}
ang <- 22 * pi / 180
u <- c(cos(ang), sin(ang))                 # direção que gera o espaço-coluna de X (esquema 1-D)
Yv <- c(4.1, 3.0)                          # vetor Y, fora do subespaço
Yhat <- as.numeric((Yv %*% u)) * u         # projeção ortogonal de Y sobre a direção u

reta <- tibble(x = c(-1, 1) * 5.4 * u[1], y = c(-1, 1) * 5.4 * u[2])

# pequeno marcador de angulo reto em Y-chapeu
perp <- (Yv - Yhat); perp <- perp / sqrt(sum(perp^2))
p1 <- Yhat + 0.22 * u
p2 <- Yhat + 0.22 * u + 0.22 * perp
p3 <- Yhat + 0.22 * perp

ggplot() +
  geom_line(data = reta, aes(x, y), color = "gray45", linewidth = 0.6) +
  geom_segment(aes(x = 0, y = 0, xend = Yv[1], yend = Yv[2]),
               arrow = arrow(length = unit(0.2, "cm")), color = "#0B4F6C", linewidth = 1) +
  geom_segment(aes(x = 0, y = 0, xend = Yhat[1], yend = Yhat[2]),
               arrow = arrow(length = unit(0.2, "cm")), color = "#2e8b57", linewidth = 1) +
  geom_segment(aes(x = Yhat[1], y = Yhat[2], xend = Yv[1], yend = Yv[2]),
               arrow = arrow(length = unit(0.2, "cm")), color = "#d98324", linewidth = 1) +
  geom_path(data = tibble(x = c(p1[1], p2[1], p3[1]), y = c(p1[2], p2[2], p3[2])),
            aes(x, y), color = "gray30", linewidth = 0.4) +
  annotate("text", x = Yv[1] + 0.25, y = Yv[2] + 0.15, label = "Y", size = 5, fontface = "italic") +
  annotate("text", x = Yhat[1] + 0.05, y = Yhat[2] - 0.35, label = expression(hat(Y)), size = 5) +
  annotate("text", x = (Yhat[1]+Yv[1])/2 + 0.35, y = (Yhat[2]+Yv[2])/2, label = "e", size = 5, fontface = "italic") +
  annotate("text", x = 5.4*u[1] + 0.1, y = 5.4*u[2] - 0.05, label = "espaço-coluna de X",
            size = 3.6, color = "gray35", hjust = 0) +
  annotate("text", x = -0.35, y = -0.25, label = "O", size = 4.2) +
  coord_equal(xlim = c(-1, 6.2), ylim = c(-1, 4.5)) +
  theme_void()
```
...
```{r plot-alavancagem, echo=FALSE, fig.width=7, fig.height=4, fig.cap="Alavancagem (diagonal de P_X) de cada observação em função do número de requisições, no modelo completo (requisições + servidor)."}
tibble(requisicoes = dados_api$requisicoes, alavancagem = diag(P)) %>%
  ggplot(aes(x = requisicoes, y = alavancagem)) +
  geom_hline(yintercept = mean(diag(P)), linetype = "dashed", color = "gray50") +
  geom_point(color = "#0B4F6C", alpha = 0.7) +
  labs(x = "Requisições simultâneas", y = expression(h[ii]~"(alavancagem)")) +
  theme_minimal(base_size = 13)
```
...
```{r fig-particao-x, echo=FALSE, fig.width=6.5, fig.height=3, fig.cap="Partição da matriz de delineamento X em dois blocos de colunas: X1 (colunas mantidas em qualquer versão do modelo, p1 delas) e X2 (colunas cuja contribuição queremos testar, p2 delas). As linhas continuam representando as n observações em ambos os blocos."}
blocos <- tibble(
  xmin = c(0, 2.3), xmax = c(2.0, 3.8), ymin = c(0, 0), ymax = c(3, 3),
  bloco = c("X[1]", "X[2]"), fill = c("#0B4F6C", "#d98324")
)
ggplot(blocos) +
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = fill), alpha = 0.75) +
  geom_text(aes(x = (xmin+xmax)/2, y = 1.5, label = bloco), parse = TRUE, color = "white", size = 7) +
  annotate("text", x = 1.0, y = -0.35, label = expression(p[1]~"colunas"), size = 4) +
  annotate("text", x = 3.05, y = -0.35, label = expression(p[2]~"colunas"), size = 4) +
  annotate("text", x = -0.55, y = 1.5, label = "n\nlinhas", size = 4, lineheight = 0.85) +
  annotate("segment", x = -0.9, xend = -0.9, y = 0, yend = 3,
           arrow = arrow(ends = "both", length = unit(0.15, "cm")), color = "gray40") +
  scale_fill_identity() +
  coord_equal(xlim = c(-1.3, 4.1), ylim = c(-0.8, 3.3)) +
  theme_void()
```
...
```{r plot-sono, echo=FALSE, fig.width=7, fig.height=4.2, fig.cap="Tempo de reação em função de horas de sono; a cor de cada ponto mostra o consumo de cafeína."}
ggplot(dados_sono, aes(x = horas_sono, y = tempo_reacao, color = cafeina_mg)) +
  geom_point(size = 2.2, alpha = 0.85) +
  geom_smooth(method = "lm", se = FALSE, color = "gray30", linewidth = 0.7, linetype = "dashed") +
...
```{r plot-fwl, echo=FALSE, fig.width=7, fig.height=4.2, fig.cap="Regressão dos resíduos: tempo de reação residualizado em horas de sono (eixo Y) contra cafeína residualizada em horas de sono (eixo X). A inclinação desta reta simples é idêntica ao coeficiente da cafeína na regressão múltipla completa."}
tibble(X2_tilde = X2_tilde, Y_tilde = Y_tilde) %>%
  ...
  labs(x = expression(tilde(X)[2]~"= cafeína residualizada em horas de sono"),
       y = expression(tilde(Y)~"= tempo de reação residualizado em horas de sono")) +
  theme_minimal(base_size = 13)
```
...
```{r monte-carlo-qui2, fig.width=7, fig.height=4}
set.seed(2026)
sigma <- 22
n_rep <- 4000
...
`r gl_residuo2` graus de liberdade — a confirmação empírica de que $\mathrm{SQE}/\sigma^2$ se
