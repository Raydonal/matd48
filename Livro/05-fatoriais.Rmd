...
O Tipo II dá exatamente $SQ_{\text{layout}}=$ `r round(anova(lm(tempo ~ desconto * layout, data = dados_desbal))["layout","Sum Sq"], 1)`
(o mesmo valor de quando `layout` entra **depois** de `desconto` no Tipo I) e
$SQ_{\text{desconto}}=$ `r round(anova(lm(tempo ~ layout * desconto, data = dados_desbal))["desconto","Sum Sq"], 1)`
...
ggplot(cubo_geo) +
  ...
  labs(title = "O 2³ como um cubo: rendimento médio de biodiesel\nem cada vértice",
       subtitle = "Sub-desenho real do 2⁴ do Capítulo 6, com o agente de\npurificação fixo no nível baixo")
```
