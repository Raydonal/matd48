...
```{r biodiesel-efeitos-plot, echo=FALSE, fig.width=7.5, fig.height=5.2, fig.cap="Os quinze efeitos estimados do fatorial 2⁴ replicado (duas repetições cada). Barras vermelhas marcam os cinco termos com p<0,05 na ANOVA completa: o efeito principal de temperatura, três interações duplas e a interação tripla dominante."}
efeitos_2k <- 2 * coef(modelo_2k)[-1]
p_2k <- broom::tidy(modelo_2k) %>%
  filter(term != "Residuals") %>%
  select(term, p.value)

tab_efeitos_2k <- tibble(termo = names(efeitos_2k), efeito = as.numeric(efeitos_2k)) %>%
  left_join(p_2k, by = c("termo" = "term")) %>%
  mutate(
    termo = str_replace_all(termo, ":", "×"),
    significativo = p.value < 0.05,
    termo = fct_reorder(termo, abs(efeito))
  )

ggplot(tab_efeitos_2k, aes(x = termo, y = efeito, fill = significativo)) +
  geom_col() +
  geom_hline(yintercept = 0, color = "grey30", linewidth = 0.4) +
  scale_fill_manual(values = c(`TRUE` = "#c0392b", `FALSE` = "grey60"), guide = "none") +
  coord_flip() +
  labs(x = NULL, y = "Efeito estimado (pontos percentuais de rendimento)",
       title = "Todos os 15 efeitos do fatorial 2⁴ replicado do biodiesel") +
...
```{r yates-dados-classicos, echo=FALSE, out.width="70%", fig.cap="Um 2³ clássico com fatores T, C e K (duas réplicas cada, I e II). As colunas de sinais (-/+) definem os oito tratamentos em ordem-padrão; as colunas à direita resumem média, amplitude e variância de cada tratamento."}
knitr::include_graphics("../Aulas2026/assets/images/dadosyates.png")
```

```{r yates-tabela-classica, echo=FALSE, out.width="65%", fig.cap="A tabela de sinais e contrastes para o 2³ acima: cada coluna (T, C, K, T x C, T x K, C x K, T x C x K) é a coluna de sinais ±1 daquele efeito, multiplicada pela Produção Média de cada tratamento (última coluna) e somada; o resultado, dividido pelo divisor do rodapé, é o efeito estimado -- exatamente o produto interno da Seção 6.1.1."}
knitr::include_graphics("../Aulas2026/assets/images/yatesout.png")
```

```{r yates-efeito-tk, echo=FALSE, out.width="55%", fig.cap="Cálculo manual do efeito de interação dupla T x C (temperatura x catalisador) no mesmo exemplo 2^3: as quatro combinações com sinal '+' (diagonal do cubo) contra as quatro com sinal '-' produzem a mesma conta que a coluna 'T x C' da tabela de sinais -- a leitura geométrica (planos diagonais do cubo, seção sobre o cubo do 2 ao cubo do Capítulo 5) e a leitura tabular (produto de colunas de sinais) chegam ao mesmo número."}
knitr::include_graphics("../Aulas2026/assets/images/Efeito_TK_yates.svg")
```
...
```{r biodiesel-halfnormal, fig.width=6.5, fig.height=5, echo=FALSE}
tab_efeitos <- tibble(termo = names(efeitos), efeito = as.numeric(efeitos)) %>%
  arrange(abs(efeito)) %>%
  mutate(
    quantil_teorico = qnorm(0.5 + 0.5 * (row_number() - 0.5) / n()),
    destaque = abs(efeito) > me
  )

ggplot(tab_efeitos, aes(x = quantil_teorico, y = abs(efeito), color = destaque)) +
  geom_point(size = 2.5) +
  ggrepel::geom_text_repel(
    data = filter(tab_efeitos, abs(efeito) > me * 0.6),
    aes(label = termo), size = 3, show.legend = FALSE, max.overlaps = 15
  ) +
  scale_color_manual(values = c("grey50", "#c0392b"), guide = "none") +
  labs(x = "Quantil teórico da meia-normal", y = "|efeito| estimado",
       title = "Gráfico de probabilidade normal dos efeitos (half-normal plot)") +
  theme_minimal(base_size = 12)
```

```{r biodiesel-pareto, fig.width=7, fig.height=5, echo=FALSE}
tab_efeitos %>%
  arrange(desc(abs(efeito))) %>%
  mutate(termo = fct_reorder(termo, abs(efeito))) %>%
  ggplot(aes(x = termo, y = abs(efeito), fill = destaque)) +
  geom_col() +
  geom_hline(yintercept = me, linetype = "dashed", color = "#c0392b") +
  annotate("text", x = 2, y = me, label = paste0("ME de Lenth = ", round(me, 1)),
           vjust = -0.6, hjust = 0, color = "#c0392b", size = 3.2) +
  scale_fill_manual(values = c("grey60", "#c0392b"), guide = "none") +
  coord_flip() +
  labs(x = NULL, y = "|efeito| estimado",
       title = "Gráfico de Pareto dos efeitos (2⁴ do biodiesel, uma repetição)") +
  theme_minimal(base_size = 12)
```
...
```{r hasse-confusao, echo=FALSE, out.width="96%", fig.cap="Esquerda: diagrama de Hasse do 2^4 de réplica única (N=16), saturado -- todos os 16 graus de liberdade já estão ocupados por Média, efeitos principais, duplas, triplas e ABCD, sem sobra para Erro nem para Bloco. Direita: para incluir um termo de Bloco (2 blocos de 8 corridas), a confusão faz Bloco e ABCD ocuparem a mesma posição relativa na estrutura -- o mesmo único grau de liberdade, com dois rótulos possíveis, em vez de um grau de liberdade roubado de outro termo."}
# Diagrama ESQUEMATICO: cada no agrega varios termos de mesma ordem, entao o gl
# e informado direto (ver a saida documentada em hasse_helpers.R) em vez de sair
# da contagem de classes.
nos_sem_bloco <- tibble(
  termo = c("Média", "Efeitos principais\n(A,B,C,D)", "Interações duplas\n(6 termos)",
            "Interações triplas\n(4 termos)", "ABCD"),
  gl    = c(1, 4, 6, 4, 1)
)
arestas_sem_bloco <- tibble(
  de   = c("Média", "Efeitos principais\n(A,B,C,D)", "Interações duplas\n(6 termos)",
           "Interações triplas\n(4 termos)"),
  para = c("Efeitos principais\n(A,B,C,D)", "Interações duplas\n(6 termos)",
           "Interações triplas\n(4 termos)", "ABCD")
)

# Confundimento se representa por um UNICO no com os dois rotulos: Bloco e ABCD
# ocupam a mesma posicao na estrutura e disputam o mesmo grau de liberdade. Por
# isso os gl continuam somando 16 = N, exatamente como no diagrama da esquerda.
nos_com_bloco <- tibble(
  termo = c("Média", "Efeitos principais\n(A,B,C,D)", "Interações duplas\n(6 termos)",
            "Interações triplas\n(4 termos)", "Bloco &#8801; ABCD\n(confundidos)"),
  gl    = c(1, 4, 6, 4, 1)
)
arestas_com_bloco <- tibble(
  de   = c("Média", "Efeitos principais\n(A,B,C,D)", "Interações duplas\n(6 termos)",
           "Interações triplas\n(4 termos)"),
  para = c("Efeitos principais\n(A,B,C,D)", "Interações duplas\n(6 termos)",
           "Interações triplas\n(4 termos)", "Bloco &#8801; ABCD\n(confundidos)")
...
knitr::include_graphics(c(
  hasse_svg(nos_sem_bloco, arestas_sem_bloco,
            arquivo = "figuras/hasse/confusao-sem-bloco.svg",
            titulo  = "Sem bloco: 2&#8308; saturado (N=16)"),
  hasse_svg(nos_com_bloco, arestas_com_bloco,
            arquivo = "figuras/hasse/confusao-com-bloco.svg",
            titulo  = "Com bloco confundido com ABCD (N=16)",
            destacar = "Bloco &#8801; ABCD\n(confundidos)")
))
```
...
```{r tabela-ab2, echo=FALSE}
crossing(A = 0:2, B = 0:2) %>%
  mutate(L = (A + 2 * B) %% 3, Bloco = paste("Bloco", L + 1)) %>%
  arrange(Bloco) %>%
  kable(caption = "Designação de blocos por AB² em um 3² (ilustração conceitual)") %>%
  kable_styling(full_width = FALSE)
```
