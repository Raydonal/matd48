...
recife <- st_read("assets/Bairros_Recife/bairros-polygon.shp", quiet = TRUE)
ggplot(recife) +
...
knitr::include_graphics("assets/images/quadro5Blocos.png")
```
...
```{r hasse-dbca-09, echo=FALSE}
source("../Livro/hasse_helpers.R")

knitr::include_graphics(
  hasse_svg(
    tibble(termo   = c("Média", "Tratamento", "Bloco", "Erro"),
           classes = c(1, 8, 3, 24)),
    tibble(de   = c("Média", "Média", "Tratamento", "Bloco"),
           para = c("Tratamento", "Bloco", "Erro", "Erro")),
    arquivo = "../Livro/figuras/hasse/slide-dbca.svg",
...
As linhas **cruzam-se** em vários pontos. Isso é sinal de não aditividade, ou é ruído? O gráfico levanta a suspeita — mas não responde.

---

## Dá para testar aditividade com uma observação por casela?

A interação **completa**, de $(t-1)(b-1)$ gl, realmente não se separa do erro: ocupa o mesmo espaço. Mas daí **não** segue que aditividade seja intestável.

--

**Teste de Tukey (1949).** Abra mão de estimar $(t-1)(b-1)$ parâmetros e postule a forma **multiplicativa**, com um único parâmetro: $(\tau\beta)_{ij}=\eta\,\tau_i\beta_j$. Sobra **1 gl** — e 1 gl cabe folgado dentro do erro.

$$SQ_N=\frac{\Big[\sum_i\sum_j y_{ij}(\bar y_{i\cdot}-\bar y_{\cdot\cdot})(\bar y_{\cdot j}-\bar y_{\cdot\cdot})\Big]^2}{\sum_i(\bar y_{i\cdot}-\bar y_{\cdot\cdot})^2 \;\sum_j(\bar y_{\cdot j}-\bar y_{\cdot\cdot})^2}, \qquad 1\text{ gl}$$

$$F_0=\frac{SQ_N}{\big(SQ_{Erro}-SQ_N\big)/[(t-1)(b-1)-1]} \;\sim\; F_{1,\,(t-1)(b-1)-1}$$

Em palavras: parte-se o erro em **não aditividade (1 gl)** e **resto**, e pergunta-se se a primeira é grande demais para ser ruído.

---

## O teste de Tukey nos dados do pepino

```{r tukey-nonadd}
mu_g <- mean(pepino$altura)
ef_t <- tapply(pepino$altura, pepino$tratamento, mean) - mu_g
ef_b <- tapply(pepino$altura, pepino$bloco,      mean) - mu_g

num  <- sum(pepino$altura * ef_t[as.character(pepino$tratamento)] *
                            ef_b[as.character(pepino$bloco)])
SQ_N <- num^2 / (sum(ef_t^2) * sum(ef_b^2))

tab  <- summary(mod_dbca)[[1]]
SQ_E <- tab["Residuals", "Sum Sq"]; gl_E <- tab["Residuals", "Df"]
F_0  <- SQ_N / ((SQ_E - SQ_N) / (gl_E - 1))
c(SQ_N = SQ_N, F = F_0, p = pf(F_0, 1, gl_E - 1, lower.tail = FALSE))
```

--

$F_{1,13}$ praticamente zero, $p\approx0{,}86$: **nenhuma evidência de não aditividade**. O cruzamento das linhas era ruído — coerente com o bloco explicar pouquíssima variação neste experimento.

**O gráfico levantou a suspeita; o teste a descartou.** É essa a divisão de trabalho, e é por isso que o diagnóstico visual nunca dispensa o formal.

--

.footnote[Ressalva: o teste só tem poder contra a forma **multiplicativa**. Em compensação, quando $\eta$ é significativo, isso costuma indicar que uma **transformação** da resposta restauraria a aditividade — diagnóstico construtivo, não só veredito.]
